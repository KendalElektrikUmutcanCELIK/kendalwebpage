...
5. **Multi-language (tr/en/ar/es/de/zh):** see "Multi-language rollout" below for full status. The rollout is complete — UI chrome, all content datasets including the product catalog, are translated into all 6 languages, no fallback-to-English gaps remain. Large per-language datasets (product catalog, news) are fetched lazily per-language from `public/` rather than statically imported, because Turbopack's file-granularity bundling would otherwise ship every language's content to every visitor — see that section for the exact mechanism before adding a new large translated dataset.
...
The rollout from 2 languages (tr/en) to 6 (+ ar/es/de/zh) is **complete**. All infrastructure and all content — including the product catalog — is fully translated into all 6 languages; there are no remaining tr/en-only fallback gaps.
...
- `src/lib/i18n/localized.ts`'s `LocalizedField<T>`/`resolveLocalized()` pattern — the standard way any *content* data type (as opposed to the UI dictionary) supports partial per-language translation with graceful fallback. Applied to: `Product` (`products.ts`), `NewsItem` (`news.json`), `settings.json`, `retailers.ts`'s `RetailCategory`, `navLinks.ts`'s `NavLink`, `pages.ts`'s `LocalizedText` (admin-panel custom page blocks), `CategoryGroupDef.name` (`products.ts`).
- **Fully translated content** (all 6 languages, no fallback needed): UI dictionary (`i18n/{tr,en,ar,es,de,zh}.json`), `settings.json`, `aboutContent.json`, `missionVision.json`, `retailers.ts`'s category names, all three `brand/*/*CreativePage.tsx` inline `translations` objects (K2/Vanti/Global) + `VantiProductFamilies.tsx`'s fan-family names, `chatbot/chatbotContent.ts` (all 4 brand-scoped Q&A trees, ~90 nodes), `news.json` (all 37 articles), **`products.json`'s `name`/`attributes`/`category` for all 793 products (5,479 attribute rows)**, `CATEGORY_GROUPS`'s two K2 menu-group names.
- **Performance work that came out of this** (independent value, not just rollout-enabling): the `products.ts`/`productsServer.ts` split and `toProductListItem()` (see the products section above), `public/product-attributes.json` lazy-fetch for the compare modal, and `public/news/{lang}.json` lazy-fetch via `newsClient.ts` — all exist because Turbopack bundles at file granularity, not per-export, so a client component importing even one unrelated function from a file that also holds a large data import (full product catalog, full 6-language news set) ships that whole file's data to the browser. **If you add a new large data file with a similar "small client-safe helpers + huge dataset" shape, budget for the same split** — it's not obvious until you inspect the actual built `_next/static/chunks/*.js` output (`grep` for a distinctive string from the data, check whether the containing chunk is referenced as an eager `<script>` in the page's exported HTML). Verified post-catalog-translation: no product-catalog strings (any language) appear in any built JS chunk, only in each product's own static HTML and in `public/product-attributes.json`.

**How the product-catalog translation was done (2026-09-18), for reference if similar bulk-translation work comes up again:** rather than translating 793 products × 3 fields × 4 languages one at a time, the actual translatable surface was much smaller than it looked — extracted to a ~870-entry TR→{ar,es,de,zh} dictionary (70 attribute labels, 50 categories, ~900 unique attribute values, ~285 unique name phrases with model-code prefixes like `GDL414`/`KES180` stripped first), since the same vocabulary (`Beyaz`, `Yerli Üretim`, `Sıva Altı`, `Tavan Vantilatörü`, dimension patterns like `Çap: X, Kalınlık: Y`, etc.) repeats across hundreds of products. Pure numeric/dimension/unit values (`10 cm x 10 cm`, `3.2V 4400mAh`, `IP65`) were deliberately left untranslated since they're already language-neutral — only strings containing actual Turkish words got dictionary entries. A Node script then applied the dictionary programmatically across the whole catalog and reported hit/miss stats, which caught the small number of gaps (all genuinely unnecessary — battery specs, chip codes) before they shipped.
